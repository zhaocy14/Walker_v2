...
class ImageEmbedding(nn.Module):
    def __init__(self, embed_dim, num_layers=5, dropout_rate=0.2):
...
if __name__ == "__main__":
    ...
    image_embedding = ImageEmbedding(embed_dim)
    motor_embedding = MotorEmbedding()
    ...
    # 生成 tgt_embed 示例输入
    tgt_embed = torch.randn(1, output_seq_length, embed_dim * 3)

    start_time = time.time()
    for i in range(10):
        image_embedded = image_embedding(images)
        motor_embedded = motor_embedding(motor_data)
        candidates = multimodal_transformer(image_embedded, motor_embedded, tgt_embed)
