...
class Odometry:
    def __init__(self, X=0.0, Y=0.0, THETA=0.0, Odo_l=0, Odo_r=0,
        ...
        self._p_l, self._p_r = 0, 0  # previous position
        self.v_l, self.v_r = 0.0, 0.0  # m/s
        self.V, self.OMEGA = 0.0, 0.0  # walker speed and angular speed
        self.Radius = 0.0   # walker turning radius
        self._X, self._Y = X, Y     # walker absolute coordinate (world coordinate system)
        self._dX, self._dY = 0.0, 0.0  # system absolute coordinate change
        self.dx, self.dy = 0.0, 0.0
        self._THETA = THETA # walker absolute direction
        self.tick_threshold = tick_threshold # threshold to count whether wheels move
        self._previous_time = time.time() # for speed calculation
        self._wheel_base = 0.68 # wheels base, distance between two wheels
        self.ratio = 5 # reduction part ratio, for example, if ratio is 5, then the wheel will move 5 times faster than the motor
    ...
    def update_pose(self, *args):
        """
        update all the odometry parameters based on the turning of the wheels
        :param args: left wheel and right wheel position
        :return:
        """
        ...
        # left wheel
        if abs(self.Odo_l - self._p_l) >= self.tick_threshold: # if changes not large enough
            # motor clock value range: 0~4294967295
            self.d_l = ((self.Odo_l - self._p_l) / 4294967296) / self.ratio * 2 * math.pi * 0.18
        else:
            self.d_l = 0
        # right wheel
        if abs(self.Odo_r - self._p_r) >= self.tick_threshold:
            self.d_r = ((self.Odo_r - self._p_r) / 4294967296) / self.ratio * 2 * math.pi * 0.18
        else:
            self.d_r = 0
